...
var UTF8 = function UTF8() {
    ...
    this.char_code_ = 0;
    this.char_length_ = 0;
    this.needed_bytes_ = 0;
    this.temp_bytes_ = new Uint8Array(4);
};
...
Object.defineProperty(UTF8.prototype, 'needed_bytes', {
    get: function get() { return this.needed_bytes_ & 0xFFFFFFFF },
    set: function get(n) { this.needed_bytes_ = n & 0xFFFFFFFF }
});

Object.defineProperty(UTF8.prototype, 'set_temp_byte', {
    get: function get() { return function(i, n) {this.temp_bytes_[i|0] = n & 0xFF} }
});

Object.defineProperty(UTF8.prototype, "set_temp_byte_length", {
    get: function get() { return function(l) {

        this.temp_bytes_ = new Uint8Array(l|0);
    } }
});

Object.defineProperty(UTF8.prototype, 'set_temp_bytes', {
    get: function get() { return function(a, i) {this.temp_bytes_.set(a, i|0)} }
});

Object.defineProperty(UTF8.prototype, 'temp_bytes_subarray', {
    get: function get() { return function(from, to) {return this.temp_bytes_.subarray(from|0, to|0)} }
});
...
UTF8.prototype._setBytesFromCharCode = function (charCode, bytes, byteOffset, neededBytes) {
    ...
    if(typeof bytes != "undefined") {
        //this.set_temp_bytes(bytes, 0);
    }

    this.needed_bytes = (neededBytes|0) == 0 ? this._getBytesForCharCode(this.char_code): neededBytes | 0;
    // Setting the charCode as it to bytes if the byte length is 1
    if (1 == (this.needed_bytes|0)) {
        this.set_temp_byte(byteOffset|0, this.char_code|0);
    } else {
        // Computing the first byte
        this.needed_bytes = this.needed_bytes-1|0;
        this.set_temp_byte(byteOffset|0, (this.FIRST_BYTE_MASK_E(this.needed_bytes) << 8 - this.needed_bytes) + (this.char_code >>> this.needed_bytes * 6) | 0);
        byteOffset = byteOffset+1|0;
        // Computing next bytes
        for (; (this.needed_bytes|0) > 0;) {
            this.needed_bytes = this.needed_bytes-1|0;
            this.set_temp_byte(byteOffset| 0, (this.char_code >>> this.needed_bytes * 6 & 0x3f | 0x80) | 0);
            byteOffset = byteOffset+1|0;
        }
    }
};
UTF8.prototype.toUint8Array = function (string, bytes, byteOffset, byteLength, strict) {
    ...
    string = string || '';
    byteOffset = byteOffset | 0;
    this.set_temp_byte_length(string.length*3+2);
    if(typeof bytes != "undefined") {
        this.set_temp_bytes(bytes, 0);
    }

    byteLength = ('number' === typeof byteLength) ? byteLength : 0 || Infinity;

    for (var i = 0, j = string.length|0; (i|0) < (j|0); i=(i+1|0)>>>0) {

        this.needed_bytes = this._getBytesForCharCode(string[i].codePointAt(0));
        //if (strict && (byteOffset + this.needed_bytes|0) > (byteLength|0)) {throw new Error('Not enought bytes to encode the char "' + string[i|0] + '" at the offset "' + byteOffset + '".');} else {
        this._setBytesFromCharCode(string[i|0].codePointAt(0), this.temp_bytes_subarray(0, byteOffset|0), byteOffset|0, this.needed_bytes|0, strict);
        byteOffset = byteOffset+this.needed_bytes|0;
        //}
    }
    return this.temp_bytes_subarray(0, byteOffset|0);
};
UTF8.prototype._getCharCode = function (bytes, byteOffset, charLength) {
    this.char_code = 0;
    byteOffset = byteOffset | 0;
    // Retrieve charLength if not given
    this.char_length = (charLength|0) == 0 ? this._getCharLength(bytes[byteOffset|0]|0): charLength | 0;

    // validate that the array has at least one byte in it
    // if (bytes.length - byteOffset <= 0) {throw new Error('No more characters remaining in array.');}

    // if ((this.char_length|0) == 0) {throw new Error(bytes[byteOffset].toString(2) + ' is not a significative' + ' byte (offset:' + byteOffset + ').');}
    ...
    // if ((bytes.length - byteOffset | 0) < (this.char_length|0)) {throw new Error('Expected at least ' + this.char_length + ' bytes remaining in array.');}

    // Test UTF8 integrity
    // if (bytes[byteOffset] & this.INTEGRITY_BYTE_MASK_E(this.char_length)) {throw Error('Index ' + byteOffset + ': A ' + this.char_length + ' bytes' + ' encoded char' + ' cannot encode the ' + (this.char_length + 1) + 'th rank bit to 1.');}
    ...
    while (this.char_length) {
        // if (0x80 !== (bytes[byteOffset + 1] & 0x80) || 0x40 === (bytes[byteOffset + 1] & 0x40)) {throw Error('Index ' + (byteOffset + 1) + ': Next bytes of encoded char' + ' must begin with a "10" bit sequence.');}
